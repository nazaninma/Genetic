...
def evolution(data, gen_size, details, tol):
    ...
    while not converged:
        ...
        old_gen = next_gen
        best_king.append(king)

        accuracy = accuracy_percentage_from_mse(data, king)
        print(f'Accuracy Percentage of Best King in Generation {gen_count}: {accuracy:.2f}%')
...
def accuracy_percentage_from_mse(data, model):
    """
    Compute accuracy percentage using MSE and variance of the true values.

    :param data: The dataset containing (input, f(x)) pairs. E.g., [(x1, f(x1)), (x2, f(x2)), ...]
    :param model: The model (tree) that has a method `evaluate(x)` which returns predictions.
    :return: Accuracy percentage as a float.
    """
    # Extract inputs (x) and true outputs (f(x))
    x_true_dataset = [x for x, fx in data]  # list of inputs
    y_true_dataset = np.array([fx for x, fx in data])  # list of true outputs (f(x))
    
    # Step 1: Compute predictions using the model
    z_cal = np.array([model.evaluate(x) for x in x_true_dataset])  # model predictions
    
    # Ensure the predicted values have the same shape as the true values
    if z_cal.shape != y_true_dataset.shape:
        print(f"Shape mismatch: z_cal {z_cal.shape}, y_true_dataset {y_true_dataset.shape}")
        return
    
    # Step 2: Calculate MSE (Mean Squared Error)
    mse = np.mean((y_true_dataset - z_cal) ** 2)
    
    # Step 3: Calculate variance of the true values
    variance_true = np.var(y_true_dataset)
    
    # Step 4: Calculate accuracy as (1 - MSE / variance_true) * 100
    if variance_true == 0:  # To avoid division by zero if all true values are the same
        accuracy = 100.0 if mse == 0 else 0.0
    else:
        accuracy = (1 - mse / variance_true) * 100
...
def new_gen(size):
    ...
    for i in range(size):
        forest.append(Tree.Tree(10))
        # calculate fitness
        forest[i].calFitness(data)
...
def tournament(pop, num_torns, data):
    ...
    for i in range(num_torns):
        ...
        for j in range(torn_size):
            fitness = pop[(i * torn_size) + j].calFitness(data)
            # see if its the best
            if fitness < best:
                best = fitness
    ...
    if len(pop) % num_torns != 0:
        ...
        for j in range(len(pop) % num_torns):
            fitness = pop[((num_torns - 1) * torn_size) + torn_size + j].calFitness(data)
            if fitness < best:
                best = fitness
...
def plot_kings_fitness(kings):
    """
    Plot the fitness of the kings (best individuals) across generations.
    
    :param kings: A list of the best individuals (kings) from each generation.
    """
    fitness_values = [king.fitness for king in kings]  # Extract the fitness values of the kings
    plt.figure(figsize=(10, 6))
    plt.plot(fitness_values, marker='o', color='b', label='King Fitness')
    plt.title('Fitness of Kings Over Generations')
...
print ('reading data')
data = readData.readData('src\dataset.csv')
